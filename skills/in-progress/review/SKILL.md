...
description: >
  ...
  standards?) and Spec (does the code match what the originating issue/PRD
  asked for?). Runs both reviews as independent passes and reports them side by
...
Both axes run as **independent passes** so they don't pollute each other's context, then this skill aggregates their findings. If Codex multi-agent tools are available, run the passes in parallel sub-agents; otherwise run them sequentially with separate notes.

The issue tracker should have been provided to you. Use the `setup-matt-pocock-skills` skill if `docs/agents/issue-tracker.md` is missing.
...
Collect the list of files. The **Standards** pass will read them.

### 4. Run both review passes

If Codex multi-agent tools are available, run the two prompts in parallel sub-agents. Otherwise, run them sequentially and keep their notes separate until aggregation.

**Standards pass prompt** - include:
